...
\begin{itemize}
    \item \textbf{Manage bookings}: The staff can view and edit the bookings created by the users. He has the ability to change the initial state of each booking (RECEIVED) to any of these: ASSIGNED (when someone has been assigned to take care of that booking), IN\_PROGRESS (when the booking is being taken care right now) and COMPLETED. Also, the booking can also be completely removed.
...
\begin{itemize}
    ...
    \item \textbf{GET /bookings/\{token\}} - Retrieve an existing booking by its public token. This is intended for check page to fetch the current details and status of their request.
    ...
    \item \textbf{GET /bookings/available-slots} - Returns a list of available time slots for a specific date and municipality, according to the bookings already set up on that day and municioality.
...
The tests rely on \texttt{JUnit 5} and \texttt{Mockito} for unit and service tests, and used Spring’s \texttt{MockMvc} for API integration testing. \texttt{Selenium WebDriver} enabled BDD-style UI automation tests. Finally, non-functional testing was implemented using \texttt{k6} scripts to stress-test the app.
...
Integration tests leveraged Spring Boot's \texttt{@SpringBootTest} and \textit{MockMvc} to test real HTTP interactions and application state via the controllers. Scenarios covered include all the main REST operations from the defined API (see Section~\ref{sec:apifordevelopers}).
...
\begin{itemize}
  ...
  \item Staff filters the bookings by municipality, seeing them both separated.
...
I also generated a \texttt{Lighthouse} report for the intial web page and got very high scores (see Fig~\ref{fig: lighthousereport}).
...
\begin{itemize}
	\item \textbf{Security}: The use of \texttt{@CrossOrigin} without restriction allowed all domains by default. This was flagged as a security and I needed to manually allow traffic from my app's controllers.
	\item \textbf{Maintainability}: There were 2 main issues. First the use of hardcodeded pauses (\texttt{Thread.sleep()}) on tests, which were replaced with condition-based waits. Second, there some logging being done via \texttt{System.err.println}, which were replaced with SLF4J logging to allow proper log levels and structured output.
\end{itemize}
